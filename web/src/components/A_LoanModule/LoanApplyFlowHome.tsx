...
export default function LoanApplyFlowHome({
  ...
  useEffect(() => {
    ;[
      ...
      .filter((d) => d.value == customerDetails?.loanBank)
      .map((d1) => {
        setLoanBank(d1)
      })
  ...
  const updatePostSancationFun = (status) => {
    ...
    const x1 = {
      LpostStatus: status || '',
      post_loan_rejection_reason:
        status === 'Rejected' ? postRejectionReason : null,
        ...
        <div className="  text-sm text-gray-700">
          <div className="mt-1">
            <div className="px-3 rounded-md py-4 mb-3 bg-[#FCFCFD] text-[14px]  flex flex-row justify-between">
              <h2 className=" flex-grow font-outfit font-normal text-[14px] leading-[18px] text-black flex-none order-0 flex-grow-0">
                Loan Approval:
              </h2>
              ...
              <p className="text-md font-outfit font-normal text-[14px] leading-[18px] text-[#EAB300] flex-none order-1 flex-grow-0">
                <div>
                  {customerDetails?.loan_rejection_reason && (
                    <p>
                      "Pre Rejection Reason:{' '}
                      {customerDetails.loan_rejection_reason}"
                    </p>
                  )}
                  {customerDetails?.post_loan_rejection_reason && (
                    <p>
                      "Post Loan Rejection Reason:{' '}
                      {customerDetails.post_loan_rejection_reason}"
                    </p>
                  )}
                  {!customerDetails?.loan_rejection_reason &&
                    !customerDetails?.post_loan_rejection_reason && (
                      <p>"Banker sanction is: {postSanctionReview}"</p>
                    )}
                </div>
              </p>
            </div>
                ...
                {[
                  {
                    ...
                    name: 'Payslips',
                    type: 'pslips',
                    time: customerDetails?.pslipsDocUpDate,
                    url: customerDetails?.pslipsDocUrl,
                    filName: customerDetails?.pslipsFilName,
                  },
                  {
                    id: 1235789,
                    name: 'Bank Statement',
                    type: 'bankstmt',
                    time: customerDetails?.bankstmtDocUpDate,
                    url: customerDetails?.bankstmtDocUrl,
                    filName: customerDetails?.bankstmtFilName,
                  },
                  {
                    id: 1236777,
                    name: 'ITR/Form-16',
                    type: 'form_16',
                    time: customerDetails?.form_16DocUpDate,
                    url: customerDetails?.form_16DocUrl,
                    filName: customerDetails?.form_16FilName,
                  },
                  {
                    id: 1236666,
                    name: 'Present Address Proof',
                    type: 'addproof',
                    time: customerDetails?.addproofDocUpDate,
                    url: customerDetails?.addproofDocUrl,
                    filName: customerDetails?.addproofFilName,
                  },
                  {
                    id: 1236999,
                    name: 'Appointment letter from HR',
                    type: 'hrletter',
                    time: customerDetails?.hrletterDocUpDate,
                    url: customerDetails?.hrletterDocUrl,
                    filName: customerDetails?.hrletterFilName,
                  },
                ]?.map((doc, i) => (
                  <section
                    key={i}
                ...
                <section className="mt-1 container">
                  {[
...

                    {
                      ...
                      name: 'Cost break-up',
                      type: 'cs',
                      time: customerDetails?.csDocUpDate,
                      url: customerDetails?.csDocUrl,
                      filName: customerDetails?.csFilName,
                    },
                    {
                      id: 2,
                      name: 'Demand Letter',
                      type: 'demandLet',
                      time: customerDetails?.demandLetDocUpDate,
                      url: customerDetails?.demandLetDocUrl,
                      filName: customerDetails?.demandLetFilName,
                    },

                    {
                      ...
                      name: 'Builder Noc',
                      type: 'bilderNoc',
                      time: customerDetails?.bilderNocDocUpDate,
                      url: customerDetails?.bilderNocDocUrl,
                      filName: customerDetails?.bilderNocFilName,
                    },
                    {
                      id: 4,
                      name: 'ATB',
                      type: 'atb',
                      time: customerDetails?.atbDocUpDate,
                      url: customerDetails?.atbDocUrl,
                      filName: customerDetails?.atbFilName,
                    },
                    {
                      id: 5,
                      name: 'TPA',
                      type: 'TPA',
                      time: customerDetails?.TPADocUpDate,
                      url: customerDetails?.TPADocUrl,
                      filName: customerDetails?.TPAFilName,
                    },
                  ]?.map((doc, i) => (
                    <section
                      key={i}
                      ...
                      <div className="mt-4">
                        <div className="">
                          <p className="text-sm text-[#0E0A1F]">
                            ...
                            {customerDetails?.loanAmount || 'No Data'}
                          </p>
                          <p className="text-sm text-[#0E0A1F]">
                            <strong>Loan Percentage:</strong>{' '}
                            {customerDetails?.loanPercentage || 'No Data'}
                          </p>
                        </div>
                      </div>
