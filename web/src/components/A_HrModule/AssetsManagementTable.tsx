        ...
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <section className="flex ml-auto mt-[18px]  bg-white  border-gray-100 py-4 rounded-t-xl px-3 ">
            {[
              ...
              <a
                key={index}
                className={`rounded-full focus:outline-none focus:ring-2   focus:bg-indigo-50 focus:ring-indigo-800 ml-3`}
                onClick={() => showOnlyDept(dat.val)}
              >
                <div
                  className={`py-1 px-7 rounded-full font-semibold hover:text-indigo-700 hover:bg-indigo-100  ${
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filterData.map((person) => (
                  <motion.tr key={person.email}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full capitalize text-green-800">
                      {person?.userStatus}
                      </span>
                    </td>
