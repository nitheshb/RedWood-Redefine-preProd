...
const EmailForm = ({
  ...
  const datee = new Date().getTime()
  const initialState = {
    customerName1: customerInfo?.customerName1 || '',

    relation1: selUnitDetails?.customerDetailsObj?.relation1 ||
      customerInfo?.relation1 || {
        label: 'S/O',
        value: 'S/O',
      },
    ...
    email1:
      selUnitDetails?.customerDetailsObj?.email1 || customerInfo?.email1 || '',

    dob1: isValidDate(customerInfo?.dob1)
      ? customerInfo?.dob1
      : isValidDate(leadPayload?.customerDetailsObj?.dob1)
      ? leadPayload?.customerDetailsObj?.dob1
      : datee,

    marital1: selUnitDetails?.customerDetailsObj?.marital1 ||
      customerInfo?.marital1 || {
        label: 'Single',
        value: 'Single',
      },
    ...
    city1:
      selUnitDetails?.customerDetailsObj?.city1 || customerInfo?.city1 || '',
    ...
    city2p:
      selUnitDetails?.customerDetailsObj?.city2p || customerInfo?.city2p || '',
    ...
    state1: selUnitDetails?.customerDetailsObj?.state1 ||
      customerInfo?.state1 || {
        value: 'KA',
        label: 'Karnataka',
      },

    state2p: selUnitDetails?.customerDetailsObj?.state2p ||
      customerInfo?.state2p || {
        value: 'KA',
        label: 'Karnataka',
      },
    ...
    panNo1:
      selUnitDetails?.customerDetailsObj?.panNo1 || customerInfo?.panNo1 || '',
          ...
          <Form className="space-y-4">
            <div>
              <section
                ...
                style={{ boxShadow: '0 1px 12px #f2f2f2' }}
              >
                <div className="w-full bg-[#EDEDED] flex flex-row justify-between mb-2 p-4 bg-white-100 rounded-t-[20px]">
                  <section className="flex flex-row">
                    <div className="w-full flex flex-col">
                      <div className="  flex flex-row gap-2 ">
                        <div>
                ...
                <section className="mt-1 px-4 mx-4 rounded-lg bg-white border border-gray-200 shadow inset-shadow-2xs">
                  ...
                  {showLeadLink && (
                    <div className="bg-[#DCD7FF] rounded-xl p-2 mt-2 flex-col">
                      <label className="text-[14px] ">
                        Auto fill from matched Leads Phone No{' '}
                      </label>
                      <section className="flex flex-row ">
                        <div className="w-full flex flex-row lg:w-3/12 px- mt-2">
                          <div className="relative w-full ">
                        ...
                        {givenPhNo1.length === 10 && (
                          <button
                            className="ml-2 mt-5 text-[12px] underline"
                            onClick={() => {
                              replaceFormData(formik)
                            }}
                          >
                  ...
                  {/* row 1 */}
                  <div className="md:flex flex-row md:space-x-4 w-full  text-sm mt-4 ">
                    <div className="space-y-2 w-full text-[16px] ">
                      <TextField
                        label="Customer Name*"
                        name="customerName1"
                        type="text"
                ...
                <section className="mt-2 px-4 mx-4 py-2 rounded-lg bg-white border-gray-200 shadow inset-shadow-2xs shadow">
                  <section className="flex justify-between">
                    ...
                    <section>
                      <div className="flex items-center mt-2">
                        <label
                          htmlFor="copyCurrentToPermanent"
                          className="ml-2 mr-2 text-sm text-[#B3B3B3]"
                        >
                          same as current address
                        </label>
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              formik.setFieldValue(
                                'address1',
                                formik.values.address2p || ''
                              )
                              formik.setFieldValue(
                                'pincode1',
                                formik.values.pincode2p || ''
                              )
                              formik.setFieldValue(
                                'city1',
                                formik.values.city2p || ''
                              )
                              formik.setFieldValue(
                                'state1',
                                formik.values.state2p || ''
                              )
                              formik.setFieldValue(
                                'countryName1',
                                formik.values.countryName2p || ''
                              )
                            }
                          }}
                        />
                    ...
                    <div className="w-full lg:w-12/12 pl-4">
                      <div className="relative w-full mb-3 mt-2">
                        <TextField
                          ...
                          type="text"
                          labelClassName="text-gray-500"
                          value={formik.values.annualIncome1.toLocaleString(
                            'en-IN'
                          )}
                          onChange={(e) => {
...
const CloneableEmailForm = ({
  ...
  useEffect(() => {
    ...
    setApplicantDetailsA(custDetailsA)
    console.log(
      'customer info selUnitDetails',
      applicantDetailsA,
      streamUnitDetails?.secondaryCustomerDetailsObj
    )
  }, [streamUnitDetails, customerInfo])
