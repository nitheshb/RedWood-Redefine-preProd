...
import { ArrowUp, ArrowDown, GripVertical, CheckCircle, XCircle } from 'lucide-react';
import { steamUsersListByRole } from 'src/context/dbQueryFirebase';
import { useAuth } from 'src/context/firebase-auth-context';
import { getLeadsAllocationOrder, upsertLeadsAllocator } from './LeadsAutoAllocator/Query';
  ...
  const unsubscribe = steamUsersListByRole(
    ...
    async (querySnapshot) => {
      ...
      usersListA.map(async (user, i) => {
        ...
        user.id = i
        const allocationOrder = await  getLeadsAllocationOrder(orgId, user.uid)
  ...
  return unsubscribe
    ...
    const sortInactiveToBottom =  (dataArray) => {
      ...
      sortedList.map((item, index) => {
        let x = item
        item.order = index +1
        upsertLeadsAllocator(orgId, item.uid, { order: index + 1, status: item.status })
        return item
      });
     fetchUsers()
      return sortedList;
    ...
    return (
      <div className="flex flex-col w-full">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {person?.status !== 'On Leave' && (
                          <button
                            ...
                            className={`ml-3 relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${person?.status === 'active' ? 'bg-blue-600' : 'bg-gray-200'}`}
                            aria-pressed={person?.status === 'active'}
                          >
                            <span className="sr-only">Toggle user status</span>
                            <span
                              className={`pointer-events-none relative inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${person?.status === 'active' ? 'translate-x-5' : 'translate-x-0'}`}
                            >
                              <span
                                className={`absolute inset-0 flex h-full w-full items-center justify-center transition-opacity ${person?.status === 'active' ? 'opacity-0 duration-100 ease-out' : 'opacity-100 duration-200 ease-in'}`}
                                aria-hidden="true"
                              >
                                <XCircle size={12} className="text-gray-400" />
                              </span>
                              <span
                                className={`absolute inset-0 flex h-full w-full items-center justify-center transition-opacity ${person?.status === 'active' ? 'opacity-100 duration-200 ease-in' : 'opacity-0 duration-100 ease-out'}`}
                                aria-hidden="true"
                              >
        ...
        <div className="mt-4 text-sm text-gray-500">
          ...
          <p>• inactive users will automatically move to the bottom of the table</p>
        </div>
      </div>
    );
  }