...
const UnitAudit = ({ title, dialogOpen, data, selUnitDetails }) => {
  ...
  const computeAmoounts = async (transactoinsA) => {
    ...
    let T_E =
      selUnitDetails?.possessionAdditionalCostCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
      }, 0) || 0
    let T_F =
      selUnitDetails?.addOnCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
    ...
    const totalUnitCost =
      (selUnitDetails?.plotCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
      }, 0) || 0) +
      (selUnitDetails?.addChargesCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
      }, 0) || 0) +
      (selUnitDetails?.constructCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
      }, 0) || 0) +
      (selUnitDetails?.constAdditionalChargesCS?.reduce(function (_this, val) {
        return (
          _this +
          (isNaN(val.TotalNetSaleValueGsT) ? 0 : val.TotalNetSaleValueGsT)
        )
