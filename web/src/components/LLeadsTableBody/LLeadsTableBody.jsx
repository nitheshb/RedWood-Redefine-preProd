...
const EnhancedTableToolbar = (props) => {
  ...
  React.useEffect(() => {
    const downRows = rowsAfterSearchKey?.map((data) => {
      let remark
      if (data?.Remarks) {
        remark =
          data?.Remarks?.charAt(0) == '-'
      ...
      return {
        Date: prettyDate(data?.Date).toLocaleString(),
        Name: data?.Name,
        countryCode: data?.countryCode,
        Mobile: data?.Mobile,
        Email: data?.Email,
        AssignedTo: data?.assignedToObj?.name,
        Source: data?.Source,
        strength: data?.leadstrength || 0,
        Status: data?.Status,
  ...
  const searchKeyField = (e) => {
    ...
    const rowsR = leadsFetchedData.filter((item) => {
      ...
      return (
        item?.Email?.toLowerCase().includes(searchString?.toLowerCase()) ||
        item?.Mobile?.toString().includes(searchString) ||
        item?.Name?.toLowerCase().includes(searchString?.toLowerCase()) ||
        item?.Project?.toLowerCase().includes(searchString?.toLowerCase()) ||
        item?.Source?.toLowerCase().includes(searchString?.toLowerCase()) ||
        item?.Status?.toLowerCase().includes(searchString?.toLowerCase())
...
export default function LLeadsTableBody({
  ...
  const [rowsPerPage, setRowsPerPage] = React.useState(5)
  const [rows, setRows] = React.useState([])
  const [searchKey, setSearchKey] = React.useState(searchVal || '')
  const [dateRange, setDateRange] = React.useState([null, null])
  const [startDate, endDate] = dateRange
  const [viewUnitStatusA, setViewUnitStatusA] = React.useState([
    'Phone No',
  ...
  const pickCustomViewer = (item) => {
    const newViewer = [...viewUnitStatusA]
    if (viewUnitStatusA.includes(item)) {
      const filtered = newViewer.filter(value => value !== item)
      setViewUnitStatusA(filtered)
    } else {
      setViewUnitStatusA([...newViewer, item])
    }
  ...
  return (
    <Section sx={{ width: '100%' }} style={{ border: 'none', radius: 0 }}>
      <EnhancedTableToolbar
        ...
        searchVal={searchVal}
      />
      <section style={{ borderTop: '1px solid #efefef', background: '#fefafb' }}>
        <TableContainer sx={{ maxHeight: 640 }}>
          <Table
            sx={{ minWidth: 750, minHeight: 260, marginBottom: 30 }}
            aria-labelledby="tableTitle"
              ...
              {leadsFetchedData
                ?.filter((item) => {
                  ...
                  return (
                    ...
                    (item.Mobile && item.Mobile.toString().includes(searchKey)) ||
                    (item.Project && item.Project.toLowerCase().includes(searchStr)) ||
                    ...
                    >
                      ...
                      <TableCell align="left">
                        <div className="relative flex flex-col group">
                          <div className="absolute bottom-0 flex-col items-center hidden mb-6 group-hover:flex"
                               style={{ zIndex: '9' }}>
                            <span className="rounded italian relative mr-2 z-100000 p-2 text-xs leading-none text-white whitespace-no-wrap bg-black shadow-lg"
                                  style={{
                                    color: 'black',
                                    background: '#e2c062',
                                    maxWidth: '300px',
                      ...
                      <TableCell align="center">
                        <div className="flex flex-col">
                          ...
                          <Rating
                            name="size-small half-rating-read"
                            value={(row?.leadstrength / 100) * 5 || 0}
                            size="small"
                            precision={0.5}
                            readOnly
                          />
