...
const LegalDocsHome = ({ project }) => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      const unsubscribe = getAllProjects(
        ...
        (querySnapshot) => {
          if (isMounted) {
            const projectsData = querySnapshot.docs.map((docSnapshot) =>
              docSnapshot.data()
            )
    ...
    if (selectedProject) {
      const fetchPhases = async () => {
        const unsubscribe = getPhasesByProject(
          ...
          (querySnapshot) => {
            if (isMounted) {
              const phasesData = querySnapshot.docs.map((docSnapshot) =>
                docSnapshot.data()
              )
    ...
    if (selectedProject && phases.length > 0) {
      const fetchBlocks = async () => {
        const unsubscribe = getBlocksByPhase(
          ...
          (querySnapshot) => {
            if (isMounted) {
              const blocksData = querySnapshot.docs.map((docSnapshot) =>
                docSnapshot.data()
              )
    ...
    if (selectedBlock) {
      const fetchUnits = async () => {
        const unsubscribe = getUnits(
          ...
          (querySnapshot) => {
            if (isMounted) {
              const unitsData = querySnapshot.docs.map((docSnapshot) =>
                docSnapshot.data()
              )
              setUnits(unitsData)
            }
  ...
  const saveFilesToDatabase = async (folderId) => {
    ...
    try {
      ...
      const uniqueFiles = files.filter(
        (file, index, self) =>
          index ===
          self.findIndex((f) => f.name === file.name && f.url === file.url)
      )
  ...
  const removeFile = async (folderId, fileName) => {
    const fileToDelete = folderFiles[folderId].find(
          ...
          >
            <div className="flex items-center gap-3">
              <img
                ...
                alt=""
                src={
                  'https://static.hsappstatic.net/ui-images/static-2.758/optimized/documents.svg'
                }
              ></img>
        ...
        <div className="mt-4">
          {currentFolder === 'projects' && (
            <div>
              ...
              <div className="mb-4">
                <label htmlFor="projectFilter" className="mr-2">
                  Filter by Project:
                </label>
                <select
                  id="projectFilter"
